...
export default async function handler(req, res) {
  ...
  const CALL_OPENER_RULES = [
    ...
    'State the specific reason for the call plainly before asking for time — reps who do this convert roughly 2x better than reps who pitch immediately or ask "got a sec?" cold. The reason itself MUST match this contact\'s seniority (see the role framing note above): a Manager/Operational contact hears a reason about cutting their own daily admin work, a Director hears one about program-wide outcomes or audit-readiness, a VP/Exec hears one about organizational risk, cost, or board-level visibility — never a one-size reason that would land the same regardless of who answered.',
  ...
  // Each category's guidance now bakes in seniority-specific framing directly
  // (not just a closing "reflect their role" instruction) because leaving
  // that to a single generic reminder produced discovery questions that read
  // basically the same whether the contact was a coordinator or a VP —
  // spelling out exactly how each pillar shifts per level makes the
  // difference concrete instead of hoping the model infers it.
  const MEDDPICC_CATEGORIES = [
    { category: 'Pain / Problem Awareness', guidance: 'Open-ended NEPQ-style questions ("what/how") that get the prospect to describe their own current pain in this contact\'s specific role/industry, in their own words — not a leading or closed question. Scale the pain itself to seniority: for a Manager/Operational contact, ask about day-to-day friction (paperwork, tracking incidents, inspection prep); for a Director, ask about inconsistency or gaps across their program/sites/teams; for a VP/Exec, ask about visibility into risk or compliance at the organizational level.' },
    { category: 'Consequence', guidance: 'Questions that get the prospect to state OUT LOUD what happens if this pain stays unfixed — this is what creates urgency, not the rep stating it for them. Scale the stakes to seniority: a Manager\'s consequence is usually personal or team-level (overtime, missed deadlines, their own frustration); a Director\'s is program-level (audit findings, inconsistent compliance across sites, team effectiveness); a VP/Exec\'s is organizational (incident cost, liability, board or investor scrutiny).' },
    { category: 'Champion & Decision Process', guidance: 'Curious, non-interrogating questions surfacing who else cares about this and how a change like this would typically get evaluated/approved at their company. For a Manager/Operational contact, ask who above them would need to be looped in or who they\'d have to convince, since they\'re rarely the one who\'d drive a purchase alone. For a Director, ask about their own role in that decision and who else needs to be aligned. For a VP/Exec, it\'s fair to ask more directly whether they\'d be the one driving or signing off on a change like this.' },
    { category: 'Economic Buyer & Metrics', guidance: 'Soft, natural questions about what success/ROI would look like or who\'d ultimately sign off on budget — framed as curiosity about their world, not a budget interrogation this early. Do not assume a Manager/Operational contact holds budget authority — ask who typically owns that decision instead of asking them directly what they\'d spend. A Director can be asked about their influence on budget or vendor decisions. A VP/Exec can be asked more directly what they\'d personally want to see to justify the investment, since they likely hold or heavily influence that budget.' },
    { category: 'Competition / Current State', guidance: 'Questions about what they use today (paper, spreadsheets, a competing tool) and what\'s working or not — if the rep\'s notes mention a specific tool the company already uses, reference it naturally here instead of asking blind. Frame the "what\'s not working" angle to seniority: day-to-day friction for a Manager, program consistency or reporting gaps for a Director, risk/visibility/cost for a VP/Exec.' }
  ];
